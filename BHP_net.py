...
def usage():
    print("BHP Net Tool                      Made by Zeus")
    print("")
    print("Usage: bhpnet.py -t target_host op port")
    print("-l --listen                       - listen on port [host]:[port] for incoming connections")
    print("-e --execute=filr_to_run          - execute a given file upon recieving a connection")
...
def main():
    ...
    if not listen and len(target) and port > 0:
        ...
        # this will block so send in CTRL-D if not sending input 
        # to stdin 
    ...
    # depending on ur command line options above
    if listen:
        server_loop()
...
def run_command(command):
    ...
    # send putput back to the client
    return output
...
def client_handler(client_socket):
    ...
    if len(upload_destination):
        ...
        # now we take these bytes and try to write them out (write binary->wb)
        try:
